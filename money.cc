...
char* coin_phrase( char_data* ch ){
      ...
      for( int i = 0; i < ch->contents; i++ ) {
            if( ( obj = object( ch->contents[i] ) ) && obj->pIndexData->item_type == ITEM_MONEY ){
      ...
      *buf = '\0';
      ...
      for( int i = MAX_COIN - 1; i >= last; --i ) {
            if( num[ i ] == 0 )
                  continue;
            int l = strlen(buf);
            snprintf( buf+l, TWO_LINES-l, "%s %s%d %s", (flag && count != 2) ? "," : "", ( i == last && flag ) ? "and " : "", num[i], coin_name[i] );
            flag = true;
      }
...
int get_money( char_data* ch ){
      ...
      for( int i = 0; i < ch->contents; i++ ) {
            for( int j = 0; j < MAX_COIN; j++ ) {
                  if( ( obj = object( ch->contents[i] ) ) && obj->pIndexData->vnum == coin_vnum[j] ) {
...
void add_coins( char_data* ch, int amount, const char *message, bool page ){
      obj_data*  obj;
      int        num  [ MAX_COIN ];
...
bool remove_coins( char_data* ch, int amount, const char *message, bool page ){
      obj_data*       obj;
      int             pos  [ MAX_COIN ];
      int             neg  [ MAX_COIN ];
...
// Count up how many coins of each type char has.
      ...
      amount = money - amount; // How much change to give?
...
/* Find a set of coins that equals the amount to be split. Start with the largest coins. */
