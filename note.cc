...
const char* noteboard_name [ MAX_NOTEBOARD ] = {
    "general", "immortal",
    "ideas", "bugs",
...
static bool can_read( char_data* ch, int i, bool msg = false )
{
    if( noteboard_level( i ) > ch->pcdata->trust ){
        if( msg ){
            fsend( ch, "You cannot read the %s noteboard.", noteboard_name[i] );
        }
    ...
    if( i == NOTE_CLAN ){
        ...
        if( !clan ){
            if( msg ){
                send( ch, "You aren't in a clan and thus have no clan noteboard.\n\r" );
            }
        ...
        if( !is_set( clan->flags, CLAN_APPROVED ) ){
            if( msg ){
                send( ch, "You cannot use the clan noteboard until your clan has been approved.\n\r" );
            }
...
Note_Data :: Note_Data( ) : next(0), from(empty_string), title(empty_string), message(empty_string), date(0), update(0), noteboard(0)
{
...
void extract( char_data* ch, note_data* note )
{
    for( int i = 0; i < player_list; i++ ){
        ...
        if( victim->Is_Valid( ) && ( victim->note_edit == note) ){
            if( victim != ch ){
                fsend( victim, "%s just deleted the note you were editing.", ch );
            }
...
void recent_notes( char_data* ch )
{
    ...
    if( recent > 0 ) {
        ...
        snprintf(
            tmp,
            TWO_LINES,
            "%s note%s been posted since last login.",
            number_word( recent ),
            recent == 1 ? " has" : "s have" );
        *tmp = toupper( *tmp );
...
static bool can_write( player_data *pc, int i, bool msg )
{
    ...
    if( is_set( pc->pcdata->pfile->flags, PLR_NO_NOTES ) && ( i != NOTE_CLAN ) ){
        if( msg )
    ...
    if( !is_apprentice( pc ) && ( i == NOTE_IMMORTAL || i == NOTE_BUILDER || i == NOTE_CHANGES || i == NOTE_CODE || i == NOTE_FIXED || i == NOTE_ANNOUNCEMENTS ) ){
        if( msg )
            fsend( pc, "Only immortals can modify the %s noteboard.", noteboard_name[i] );
    ...
    if( i == NOTE_AVATAR && !is_avatar( pc )  ){
        if( msg )
            fsend( pc, "You aren't an avatar and cannot modify the %s board.", noteboard_name[i] );
        return false;
    }
    ...
    if( !is_apprentice( pc ) && ( pc->gossip_pts < 0 ) && ( i != NOTE_CLAN ) ){
        if( msg )
...
static bool can_remove( player_data *pc, note_data *note, const char *msg )
{
    ...
    if( ( note->noteboard == NOTE_AVATAR ) && is_avatar( pc ) )
        return true;
    
    // Author can edit own notes.
    // Noteboard perms allow edit of any note.
    if( !strcmp( pc->descr->name, note->from ) || has_permission( pc, PERM_NOTEBOARD ) )
        return true;
    
    title_data *title;
    
    if( ( note->noteboard == NOTE_CLAN ) && ( title = get_title( pc->pcdata->pfile ) ) && is_set( title->flags, TITLE_REMOVE_NOTES ) )
        return true;
...
void do_notes( char_data* ch, const char *argument )
{
    ...
    if( !( note = pc->note_edit ) ){
        ...
        if( matches( argument, "move" ) ) {
            ...
            if( strcmp( ch->descr->name, note->from ) && !has_permission( ch, PERM_NOTEBOARD ) ) {
                send( ch, "You are unable to move that note.\n\r" );
                return;
            }
            
            if( noteboard == pc->noteboard ) {
                fsend( ch, "The note is already on the %s board.", noteboard_name[noteboard] );
                return;
            }
            
            remove( note_list[ pc->noteboard ], max_note[ pc->noteboard ], i );
            insert( note_list[noteboard], max_note[noteboard], note, max_note[noteboard] );
            fsend( ch, "Note \"%s\" moved to %s board.", note->title, noteboard_name[noteboard] );
            
            snprintf( tmp1,
                    3*MAX_STRING_LENGTH,
                    "Note \"%s\" moved by %s to %s board from %s board.",
                    note->title,
                    ch->descr->name,
            ...
            snprintf( tmp2,
                    THREE_LINES,
                    "Note \"%s\" moved by a mail daemon to %s board from %s board.",
                    note->title,
                    noteboard_name[ noteboard ],
                    noteboard_name[ pc->noteboard ]
            );
            info( min( noteboard_level( noteboard ),
                    noteboard_level( pc->noteboard ) ),
                    tmp2,
                    invis_level( pc ),
                    tmp1,
                    MAX_IFLAG+noteboard,
        ...
        if( matches( argument, "delete" ) ) {
            if( !strcasecmp( argument, "all" ) ) {
                ...
                if( max_note[noteboard] == 0 ){
                    fsend( ch, "The %s noteboard is already clear.", noteboard_name[noteboard] );
                    return;
                }
                ...
                fsend( ch, "You clear the %s noteboard.", noteboard_name[noteboard] );
                snprintf( tmp1, 3*MAX_STRING_LENGTH, "%s noteboard cleared by %s.", noteboard_name[noteboard], ch->real_name( ) );
                snprintf( tmp2, THREE_LINES, "%s noteboard cleared by a mail daemon.", noteboard_name[noteboard] );
                *tmp1 = toupper( *tmp1 );
                *tmp2 = toupper( *tmp2 );
                info( noteboard_level( noteboard ), tmp2, invis_level( pc ), tmp1, MAX_IFLAG+noteboard, 1, ch );
                save_notes( noteboard, clan );
            ...
            if( count == 1 ) {
                ...
                fsend( ch, "Note \"%s\" removed from %s board.", note->title, noteboard_name[noteboard] );
                snprintf( tmp1, 3*MAX_STRING_LENGTH, "Note \"%s\" deleted from the %s board by %s.", note->title, noteboard_name[noteboard], ch->descr->name );
                snprintf( tmp2, THREE_LINES, "Note \"%s\" deleted from the %s board by a mail daemon.", note->title, noteboard_name[noteboard] );
                extract( ch, note );
            }
            else {
                ...
                fsend( ch, "%s notes removed from %s board.", number_word( count, ch ), noteboard_name[noteboard] );
                snprintf( tmp1, 3*MAX_STRING_LENGTH, "%s notes deleted from the %s board by %s.", number_word( count, ch ), noteboard_name[noteboard], ch->descr->name );
                snprintf( tmp2, THREE_LINES, "%s notes deleted from the %s board by a mail daemon.", number_word( count, ch ), noteboard_name[noteboard] );
            }
            
            *tmp1 = toupper( *tmp1 );
            *tmp2 = toupper( *tmp2 );
            info( noteboard_level( noteboard ), tmp2, invis_level( pc ), tmp1, MAX_IFLAG+noteboard, 3, ch, noteboard == NOTE_CLAN ? ch->pcdata->pfile->clan : 0 );
            save_notes( noteboard, clan );
            return;
        }
        ...
        if( isdigit( *argument ) ) {
            ...
            page( ch, "   Title: %s\n\r    From: %s\n\r  Posted: %s\n\r", note->title, note->from, ltime( note->date, false, ch ) );
            if( note->update != 0 ) {
                page( ch, "Modified: %s\n\r", ltime( note->update, false, ch ) );
            }
            page( ch, "\n\r" );
        ...
        if( matches( argument, "edit" ) ) {
            ...
            if( ( --number < 0 ) || ( number >= max_note[noteboard] ) ) {
                send( ch, "There is no note by that number.\n\r" );
                return;
            }
        ...
        if( matches( argument, "search" ) ) {
            ...
            for( int n = 0; n < max; ++n ) {
                ...
                if( search_text( note->from, argument ) || search_text( note->title, argument ) || search_text( note->message, argument ) ) {
                    if( count == 0 ) {
        ...
        if( !exact_match( argument, "new" ) && ( ( i = find_noteboard( ch, argument ) ) != -1 ) ) {
            if( can_read( ch, i, true ) ) {
                ...
                // Switch noteboards.
                pc->noteboard = i;
                fsend( ch, "Note now works on the %s noteboard. ( %d notes )", noteboard_name[i], max_note[i] );
            }
    ...
    if( exact_match( argument, "title" ) ) {
        if( !can_write( pc, note->noteboard, true ) || !acceptable_title( ch, argument ) )
            return;
    ...
    if( !strcasecmp( argument, "post" ) ) {
        ...
        if( note->noteboard == -1 ) {
            note->noteboard = noteboard;
            fsend( ch, "Note \"%s\" posted on %s board.", note->title, noteboard_name[noteboard] );
            snprintf( tmp1, 3*MAX_STRING_LENGTH, "Note \"%s\" posted by %s on the %s board.", note->title, ch->descr->name, noteboard_name[noteboard] );
            info( noteboard_level( noteboard ), empty_string, 0, tmp1, MAX_IFLAG+noteboard, 1, ch, noteboard == NOTE_CLAN ? ch->pcdata->pfile->clan : 0 );
            note->date = current_time;
        }
        else {
            fsend( ch, "Modifications to note \"%s\" saved on %s board.", note->title, noteboard_name[ note->noteboard ] );
            snprintf( tmp1, 3*MAX_STRING_LENGTH, "Note \"%s\" on %s board modified by %s.", note->title, noteboard_name[ note->noteboard ], ch->descr->name );
            snprintf( tmp2, THREE_LINES, "Note \"%s\" on %s board modified by a mail daemon.", note->title, noteboard_name[ note->noteboard ] );
            info( noteboard_level( noteboard ), tmp2, invis_level( pc ), tmp1, MAX_IFLAG+noteboard, 2, ch, noteboard == NOTE_CLAN ? ch->pcdata->pfile->clan : 0 );
    ...
    if( *argument && !can_write( pc, note->noteboard, true ) )
        return;
    ...
    pc->note_edit->message = edit_string( ch, argument, pc->note_edit->message, MEM_NOTE, true );
}

void display_note( char_data *ch, int i, const note_data *note )
{
    page( ch, "[%3d] %-15s %-40s %s\n\r", i, note->from, note->title, ltime( note->Date( ), false, ch ) );
}
...
"The mail daemon takes the letter, opens it, reads it carefully, rolls on the floor \
laughing, and wanders off to find %s.";
...
tune.  When it realizes you aren't going to pay postage, it stamps around just out \
of reach and mumbles rudely about immortals, before slowly wandering off in the \
wrong direction to find %s.";

static const char *const message4 =
"You give a mail daemon your letter.  It stands nearby with its hand out for a few \
minutes and then grumbles and wanders off to find %s, while chewing on a corner of \
the letter.";

static const char *const message5 =
"The mail daemon looks at you, then at %s. It thumps you on the head, hands your \
letter to %s, and wanders away grumbling.";
...
void mail_message( char_data* ch )
{
    ...
    if( total != 0 ) {
        send_centered( ch, "You have %s mail message%s.", number_word( total ), total == 1 ? "" : "s" );
    }
}
...
static void send_mail( char_data* ch, char letter, const char *argument )
{
    ...
    if( ch->pcdata->pfile != pfile ) {
        ...
        if( ch->Filtering( pfile ) ) {
            fsend( ch, "You are filtering %s and only a chebucto would want to send mail to with someone they are filtering.",
            pfile->name );
            return;
        }
        if( !remove_silver( ch ) ) {
            ...
            fsend( ch, number_range( 0, 1 ) == 0 ? message3 : message4, pfile->name );
            ...
            && ch->Recognizes( pl ) ) {
                ...
                fsend( ch, number_range( 0, 25 ) == 0 ? message1 : message2, pfile->name );
            }
        }
    }
    else {
        fsend( ch, "You %s the letter to yourself.", letter == 'S' ? "send" : "cc a copy of" );
        pl = player( ch );
    }
...
void do_mail( char_data* ch, const char *argument )
{
    ...
    if( !( note = ch->pcdata->mail_edit ) ) {
        ...
        if( exact_match( argument, "delete" ) ) {
            ...
            if( !strcasecmp( argument, "all" ) ) {
                ...
                fsend( ch, "Mail message \"%s\" deleted.", note->title );
                delete note;
            }
    ...
    ch->pcdata->mail_edit->message = edit_string( ch, argument, ch->pcdata->mail_edit->message, MEM_NOTE, true );
}
