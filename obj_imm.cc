...
const char *layer_name [ MAX_LAYER+1 ] = { "any", "bottom", "under", "base", "over", "top" };
...
const char* restriction_flags [ MAX_RESTRICTION ] = { "bladed", "no_hide", "no_sneak", "dishonorable" };

const char* restrict_ident [ MAX_RESTRICTION ] = { "bladed", "can't hide", "can't sneak", "dishonorable" };
...
static const char *const no_permission = "You don't have permission to alter that object.\n\r";
...
static int select_ofind( obj_clss_data* obj, char_data* ch, const char *argument )
{
	...
	while( true ) {
		...
		if( hyphen != '-' ) {
			...
			++argument;
			if( !isalpha( letter = *argument++ ) && ( letter != '#' ) ) {
		...
		switch( letter ) {
			case 't':
				string = item_type_name[ obj->item_type ];
				break;
			case 'c':
				string = obj->creator;
				break;
		...
		if( letter == 'l' ) {
			...
			if( max < 0 )
				max = INT_MAX;
			if( negative ^ ( ( obj->level < min ) || ( obj->level > max ) ) )
				return 0;
		...
		if( letter == '#' ) {
			...
			if( max < 0 )
				max = INT_MAX;
			if( negative ^ ( ( obj->limit < min ) || ( obj->limit > max ) ) )
				return 0;
		...
		if( letter == 'T' ) {
			...
			for( oprog = obj->oprog; oprog && oprog->trigger != i; oprog = oprog->next );
...
static void display_ofind( obj_clss_data* obj, char_data* ch, char* buf, int& length )
{
	...
	switch( obj->item_type ) {
		case ITEM_ARMOR :
			length += snprintf( buf+length, MAX_STRING_LENGTH, "AC: %d", obj->value[1] );
			break;
		case ITEM_WEAPON :
		case ITEM_ARROW :
		case ITEM_TRAP :
			length += snprintf( buf+length, MAX_STRING_LENGTH, "Dm: %s", dice_string( obj->value[1] ) );
			break;
	}
...
void do_identify( char_data* ch, const char *argument )
{
	...
	if( !( obj = one_object( ch, argument, "identity", &ch->contents, &ch->wearing, ch->array ) ) ) 
		return;
...
void do_oedit( char_data* ch, const char *argument )
{
	...
	if( number_arg( argument, i ) ) {
		...
		if( !( obj = one_object( ch, argument, "oedit", &ch->contents, &ch->wearing, ch->array ) ) )
			return;
	...
	/* Perform token ostat for object just selected */
	page( ch, "\n\r" );
	...
	page( ch, "          Vnum: %-10d Creator: %s%-13s%s Type: %s\n\r",
		obj_clss->vnum,
		color_code( ch, COLOR_BOLD_YELLOW ),
	...
	page( ch, "         Fakes: (%d) %s\n\r",
		obj_clss->fakes, !( fakes = get_obj_index( obj_clss->fakes ) ) ? "none" : fakes->Name( )
	);
	...
	switch( obj_clss->item_type ) {
		...
		case ITEM_TRAP   :
			page( ch, "\n\r" );
			page( ch, "        Damage: %-26s\n\r", dice_string( obj_clss->value[1] ) );
			break;
		...
		case ITEM_POTION :
		{
			...
			page( ch, "         Empty: %d (%s)\n\r",
				obj_clss->value[3],
				!( empty = get_obj_index( obj_clss->value[3] ) ) ? "none" : empty->Name( )
			);
		}
		break;
		...
		case ITEM_CONTAINER :   
		{
			...
			page( ch, "      Capacity: %-14d Key: %d (%s)\n\r",
				obj_clss->value[0],
				obj_clss->value[2],
				!( key = get_obj_index( obj_clss->value[2] ) ) ? "none" : key->Name( )
			);
		}
		break;
		...
		case ITEM_DRINK_CON :
		{
			...
			page( ch, "      Capacity: %-10d Contains: %-10d Liquid: %s\n\r",
				obj_clss->value[0],
				obj_clss->value[1],
				( liquid < 0 || liquid > table_max[ TABLE_LIQUID ] ) ? "unknown" : liquid_name( liquid, true )
			);
		}
		break;
		...
		case ITEM_GATE :
		{
			...
			page( ch, "      Lifetime: %-15d To: %d (%s)\n\r",
				obj_clss->value[0],
				obj_clss->value[1],
				!( to = get_room_index( obj_clss->value[1], false ) ) ? "none" : to->name
			);
		}
		break;
	...
	if( ( obj_clss->fakes != 0 ) && ( obj_clss->fakes != obj_clss->vnum ) ) {
		sprintf( tmp, "\n\r[ before ][ from Obj %d ]\n\r%s\n\r",
			obj_clss->fakes,
			before_descr( obj_clss )
		);
		page( ch, tmp );
	}   
...
bool obj_clss_arg( char_data *ch, const char *& argument, obj_clss_data *clss, obj_data *obj, char *name, int& val )
{
...
void do_ostat( char_data* ch, const char *argument )
{
	...
	if( !*argument ) {
		...
		if( !( obj = one_object( ch, argument, "ostat", &ch->contents, &ch->wearing, ch->array ) ) )
			return;
...
static int select_owhere( obj_clss_data* obj_clss, obj_data* obj, char_data* ch, const char *argument )
{
	char               tmp  [ MAX_INPUT_LENGTH ];
	char            letter;
	char            hyphen;
...
static void display_owhere( obj_data* obj, char_data* ch, char* tmp, int& length )
{
	room_data*          room  = 0;
	char_data*         owner  = 0;   
...
static unsigned oiflag_support( char_data *ch, char_data *pl, Content_Array& stuff, const obj_clss_data *clss, int bit, int flag, bool val, unsigned& count )
{
	if( stuff.is_empty() )
    return 0;
